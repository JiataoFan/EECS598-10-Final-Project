...
function init() {
	...
	for (var p = 0; p < pointcloud.length; p++) {

		// create a particle with position values
		var pX = pointcloud[p][1],
			pY = pointcloud[p][2],
			pZ = pointcloud[p][0],
			particle = new THREE.Vector3(pX, pY, pZ);

		color = new THREE.Color(Math.round(p/pointcloud.length*255));
